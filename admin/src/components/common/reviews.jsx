...
function getEntityId(review, entityType) {
  ...
  if (nested && typeof nested === 'object')
    return nested._id ? String(nested._id) : null;
...
function Reviews({ id, type }) {
  ...
  const otherEntities = useMemo(() => OTHER_ENTITIES[type] || [], [type]);
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      try {
        ...
        setReviews([]);
        setError(
          requestError.message || 'Unable to load reviews. Please try again.'
        );
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    otherEntities.forEach((entityType) => {
      ...
      const uniqueIds = [
        ...new Set(
          reviews
            .map((review) => getEntityId(review, entityType))
            .filter(Boolean)
        ),
      ];
      const idsToFetch = uniqueIds.filter(
        (entityId) => !(`${entityType}:${entityId}` in entityNames)
      );
    ...
    reviews.forEach((review) => {
      map[review._id] = otherEntities.map((entityType) => {
        ...
        return {
          entityType,
          name: name || 'Loading…',
          label: ENTITY_CONFIG[entityType].nounLabel,
        };
      });
    });
    return map;
  ...
  const filteredReviews = useMemo(() => {
    ...
    return reviews.filter((review) => {
      ...
      const nameFields = (reviewMeta[review._id] || []).map(
        (meta) => meta.name
      );
  ...
  const handleEditSave = async () => {
    ...
    try {
      const payload = {
        ...
        difficultyRating:
          editForm.difficultyRating === ''
            ? undefined
            : Number(editForm.difficultyRating),
      ...
      setSaveError(
        updateError.message || 'Unable to save changes. Please try again.'
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setReviews((prev) =>
        prev.filter((review) => review._id !== deletingReview._id)
      );
      setDeletingReview(null);
      <Toast type="success" message="Review deleted successfully" />;
    } catch (deleteErr) {
      setDeleteError(
        deleteErr.message || 'Unable to delete this review. Please try again.'
      );
    } finally {
      setDeleting(false);
    }
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="mt-1 text-2xl font-black text-white sm:text-3xl">
                  {label} reviews
                  {subjectName && (
                    <span className="text-cyan-200"> · {subjectName}</span>
                  )}
                </h1>
              </div>
            </div>
            <div className="rounded-xl border border-cyan-300/15 bg-slate-950/50 px-4 py-2 text-sm text-slate-300">
              <span className="font-bold text-cyan-200">{reviews.length}</span>{' '}
              total
            </div>
          </div>
        ...
        {error ? (
          ...
          <div className="mt-6 grid gap-4">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="rounded-2xl border border-cyan-300/10 bg-slate-900/70 p-5"
              >
          ...
          <div className="mt-6 grid gap-4">
            {filteredReviews.map((review) => (
              <article
                key={review._id}
                className="group rounded-2xl border border-cyan-300/10 bg-slate-900/70 p-5 shadow-lg shadow-slate-950/20 transition hover:border-cyan-300/20"
              >
        ...
        <DialogContent className="border-cyan-300/15 bg-slate-900 text-slate-50 sm:max-w-md">
          ...
          <FieldGroup>
            <div className="grid grid-cols-2 gap-4">
              <Field>
                <FieldLabel htmlFor="edit-rating" className="text-slate-400">
                  Rating
                </FieldLabel>
                <Input
                  id="edit-rating"
            ...
            <Field>
              <FieldLabel htmlFor="edit-semester" className="text-slate-400">
                Semester
              </FieldLabel>
              <Input
                id="edit-semester"
            ...
            <Field>
              <FieldLabel htmlFor="edit-comment" className="text-slate-400">
                Comment
              </FieldLabel>
              <Textarea
                id="edit-comment"
