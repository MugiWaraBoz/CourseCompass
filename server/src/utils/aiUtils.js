function buildReviewSummaryInput(reviewData) {
    ...
    const avgRating = avg(reviewData.map(review => review.rating).filter(rating => rating != null));
    const avgDifficultyRating = avg(reviewData.map(review => review.difficultyRating).filter(rating => rating != null));

    const semester = [...new Set(reviewData.map(review => review.semester).filter(sem => sem != null))].join(", "); 
...
function generatePrompt(reviewData, summaryData, entityType) {
    const { count, avgRating, avgDifficultyRating, semester, comments } = summaryData;
    const entityName = (entityType === "course") ? reviewData[0]?.courseName : reviewData[0]?.facultyName || "Unknown";
    const relatedLabel = entityType === "course" ? "Faculties" : "Courses";
    const relatedNames = [...new Set(reviewData.map(review => entityType === "course" ? review.facultyName : review.courseName).filter(name => name != null))];
